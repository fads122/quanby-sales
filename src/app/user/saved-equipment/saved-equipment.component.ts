...
export class SavedEquipmentComponent implements OnInit {
  ...
  async ngOnInit() {
    await this.loadSavedEquipment();
  }

  async loadSavedEquipment() {
    try {
      this.savedEquipment = await this.supabaseService.getSavedEquipment();
      this.savedEquipment = this.savedEquipment.map(entry => ({
        ...entry,
        groupedItems: this.groupEquipment(entry.items)
      }));
    } catch (error) {
      alert('Failed to load saved equipment.');
    }
  }
  ...
  async clearSpecificSavedEquipment(entryToRemove: any) {
    const confirmDelete = window.confirm("❗ Are you sure you want to delete this saved equipment?");
    if (confirmDelete) {
      try {
        await this.supabaseService.deleteSavedEquipment(entryToRemove.id);
        await this.loadSavedEquipment();
  ...
  // When saving equipment:
  async saveEquipmentToSupabase(entry: any) {
    try {
